---
title: Plugin
description: ขยายความสามารถของ Opencode ด้วย Plugin และ Hooks
order: 1
---

import ChatMessage from "@/components/ChatMessage.astro"

## Plugin คืออะไร

Plugin คือส่วนขยายที่ช่วยให้คุณสามารถเพิ่มความสามารถให้กับ Opencode ได้ โดยการเชื่อมโยงกับเหตุการณ์ (events) ต่างๆ และปรับแต่งลักษณะการทำงานของมัน

## วิธีการติดตั้ง Plugin

มี 2 วิธีหลักในการโหลด Plugin:

### จากไฟล์ในเครื่อง

วางไฟล์ JavaScript หรือ TypeScript ในไดเร็กทอรี Plugin:

- `.opencode/plugins/` - Plugin ระดับโครงการ
- `~/.config/opencode/plugins/` - Plugin ระดับ Global

### จาก npm

ระบุแพ็คเกจ npm ในไฟล์ปรับแต่งของคุณ:

```json
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": [
    "opencode-helicone-session",
    "opencode-wakatime",
    "@my-org/custom-plugin"
  ]
}
```

---

## Hooks คืออะไร

Hooks คือจุดเชื่อมต่อที่ช่วยให้ Plugin สามารถ "แทรก" ตัวเองเข้าไปในกระบวนการทำงานของ Opencode ได้ ทำให้สามารถ:

- ตรวจสอบการทำงานก่อน/หลังที่เกิดขึ้น
- แก้ไขข้อมูลระหว่างทาง
- ส่งการแจ้งเตือนเมื่อมีเหตุการณ์บางอย่างเกิดขึ้น

### ตัวอย่าง Hooks ที่ใช้บ่อย

| Hook | คำอธิบาย |
|------|---------|
| `session.created` | เมื่อสร้าง session ใหม่ |
| `session.idle` | เมื่อ session ว่างาน |
| `tool.execute.before` | ก่อนเรียกใช้ tool |
| `tool.execute.after` | หลังเรียกใช้ tool |
| `shell.env` | ก่อนรัน shell command |
| `file.edited` | เมื่อแก้ไขไฟล์ |

---

## ตัวอย่างการสร้าง Plugin

### 1. ส่ง Notification เมื่อสร้าง Session สำเร็จ

```javascript
// .opencode/plugins/notification.js
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      if (event.type === "session.created") {
        await $`osascript -e 'display notification "Session started!" with title "opencode"'`
      }
    },
  }
}
```

### 2. ป้องกันการอ่านไฟล์ .env

```javascript
// .opencode/plugins/env-protection.js
export const EnvProtection = async ({ project, client, $, directory, worktree }) => {
  return {
    "tool.execute.before": async (input, output) => {
      if (input.tool === "read" && output.args.filePath.includes(".env")) {
        throw new Error("Do not read .env files")
      }
    },
  }
}
```

<ChatMessage message="การป้องกัน .env เป็นสิ่งสำคัญมาก! ป้องกันไม่ให้ AI อ่าน secrets ของคุณ" avatarKey="blackCat" />

### 3. ฉีด Environment Variables

```javascript
// .opencode/plugins/inject-env.js
export const InjectEnvPlugin = async () => {
  return {
    "shell.env": async (input, output) => {
      output.env.MY_API_KEY = "secret"
      output.env.PROJECT_ROOT = input.cwd
    },
  }
}
```

---

## โครงสร้างพื้นฐานของ Plugin

Plugin คือ JavaScript/TypeScript module ที่ส่งออก function อย่างน้อยหนึ่งรายการ:

```javascript
// .opencode/plugins/example.js
export const MyPlugin = async ({ project, client, $, directory, worktree }) => {
  console.log("Plugin initialized!")

  return {
    // Hook implementations go here
  }
}
```

### พารามิเตอร์ที่ได้รับ

- `project`: ข้อมูลโครงการปัจจุบัน
- `directory`: ไดเร็กทอรีการทำงานปัจจุบัน
- `worktree`: เส้นทางเวิร์กทรีคอมไพล์
- `client`: ไคลเอนต์ Opencode SDK
- `$`: shell API ของ Bun

### รองรับ TypeScript

```typescript
import type { Plugin } from "@opencode-ai/plugin"

export const MyPlugin: Plugin = async ({ project, client, $, directory, worktree }) => {
  return {
    // Type-safe hook implementations
  }
}
```

---

## สรุป

Plugin และ Hooks เป็นเครื่องมือที่ทรงพลังมากสำหรับการปรับแต่ง Opencode ให้เหมาะกับ workflow ของคุณ

<ChatMessage message="เมื่อเข้าใจ Plugin แล้ว คุณจะสามารถปรับ Opencode ให้เป็นเครื่องมือที่เหมาะกับคุณได้เลย!" avatarKey="whiteCat" />

import { Quiz } from "@/components/quiz/Quiz"

export const quizPlugin = {
  id: "plugin-quiz",
  title: "ทดสอบความเข้าใจเกี่ยวกับ Plugin",
  questions: [
    {
      id: "q1",
      type: "single",
      question: "Plugin ใน Opencode ทำหน้าที่อะไร?",
      options: ["เก็บข้อมูล session", "ขยายความสามารถของ Opencode", "จัดการไฟล์", "ควบคุม network"],
      correctAnswer: "ขยายความสามารถของ Opencode",
      points: 1
    },
    {
      id: "q2",
      type: "single",
      question: "Plugin ระดับโครงการอยู่ที่ใด?",
      options: ["~/.config/opencode/plugins/", ".opencode/plugins/", "/usr/local/plugins/", "~/plugins/"],
      correctAnswer: ".opencode/plugins/",
      points: 1
    },
    {
      id: "q3",
      type: "single",
      question: "Hooks ทำหน้าที่อะไร?",
      options: ["เก็บข้อมูล user", "จุดเชื่อมต่อที่ช่วยให้ Plugin แทรกตัวเองเข้าไปในกระบวนการทำงาน", "รัน shell commands", "จัดการ session"],
      correctAnswer: "จุดเชื่อมต่อที่ช่วยให้ Plugin แทรกตัวเองเข้าไปในกระบวนการทำงาน",
      points: 1
    },
    {
      id: "q4",
      type: "multiple",
      question: "ตัวอย่าง Hooks ที่ใช้บ่อยมีอะไรบ้าง?",
      options: ["session.created", "session.idle", "tool.execute.before", "file.edited", "network.error"],
      correctAnswer: ["session.created", "session.idle", "tool.execute.before", "file.edited"],
      points: 1
    },
    {
      id: "q5",
      type: "single",
      question: "Plugin ถูกเขียนด้วยภาษาใด?",
      options: ["Python", "JavaScript/TypeScript", "Go", "Rust"],
      correctAnswer: "JavaScript/TypeScript",
      points: 1
    }
  ]
}

<Quiz quiz={quizPlugin} client:load />